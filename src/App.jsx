...
function App() {
  ...
  return (
    <main className='bg-dgray-900 w-full min-h-screen flex justify-center items-center font-sans'>
      ...
      <section className="w-[90%] max-w-[330px] bg-dgray-800 py-6 px-8 rounded-xl ">
        ...
        <div className='mt-5'>
          {socialLinks.map((soc,i) => {
            return (
              <a href='#' className='text-white bg-dgray-700 mb-4 text-xs text-center py-3 rounded-lg cursor-pointer block hover:bg-dgreen hover:text-dgray-900' key={i}>{soc} </a>
            )
          })}
        </div>
      </section>
